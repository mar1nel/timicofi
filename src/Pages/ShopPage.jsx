...
export default function ShopPage() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const [coffeesRes, stockRes] = await Promise.all([
                    fetch("http://localhost:8080/reports/coffees"),
                    fetch("http://localhost:8080/reports/stock"),
                ]);
                if (!coffeesRes.ok) throw new Error(await coffeesRes.text());
                if (!stockRes.ok) throw new Error(await stockRes.text());

                const [coffees, stockArray] = await Promise.all([
                    coffeesRes.json(),
                    stockRes.json(),
                ]);

                const stockMap = stockArray.reduce((m, {id, stock}) => {
                    m[id] = stock;
                    return m;
                }, {});

                setProducts(coffees.map(c => ({
                    ...c,
    ...
    return (
        <>
            ...
            <section className="wrapper-shop-upper">
            ...
            {outOfStock.length > 0 && (
                <section className="wrapper-shop-down">
